...
namespace QuanLyKho
{
    public partial class Employees_Store : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void loadData()
        {
            try
            {
                gctEmployees.DataSource = EmployessController.Instance.DataSource_GetEmployees();
            }
            catch { }
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (action == 1)//add
            {
                EmployessController.Instance.InsertEmployees(EmployeesName, UserName, Phone, Country, DateOfBirth, Salary);
            }
            if (action == 2)//update
            {
                EmployeesID = Convert.ToInt32(tbEmployeesID.Text.Trim(' '));
                EmployessController.Instance.UpdateEmployees(EmployeesID, Salary);
            }
            if (action == 3)//delete
            {
                EmployeesID = Convert.ToInt32(tbEmployeesID.Text.Trim(' '));
                EmployessController.Instance.DeleteEmployees(EmployeesID);
            }
            EnabledBtn(true);
